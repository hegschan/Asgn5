#!/usr/bin/env python3
"""Generate seamless tileable waterfall and cliff textures (original art)."""
...
def seamless_noise(x: np.ndarray, y: np.ndarray, seed: float = 0.0) -> np.ndarray:
    """Tileable value noise using periodic sin combinations."""
    return (
...
def generate_cliff(w: int = 1024, h: int = 1024) -> Image.Image:
    xs = np.linspace(0, 1, w, dtype=np.float32)
    ys = np.linspace(0, 1, h, dtype=np.float32)
    x, y = np.meshgrid(xs, ys)

    n1 = seamless_noise(x * 3, y * 3, 0.5)
    n2 = seamless_noise(x * 9, y * 9, 1.8)
    n3 = seamless_noise(x * 18, y * 18, 3.2)
    height = n1 * 0.55 + n2 * 0.3 + n3 * 0.15

    moss = seamless_noise(x * 5 + 0.2, y * 4, 4.4)
    moss_mask = np.clip((0.55 - height) * 2.5 + moss * 0.35, 0, 1)

    r = np.clip(75 + height * 55 + moss_mask * 25, 0, 255)
    g = np.clip(68 + height * 45 + moss_mask * 55, 0, 255)
    b = np.clip(58 + height * 35 + moss_mask * 20, 0, 255)
...
def main() -> None:
    ...
    cliff = OUT / "cliff_rock.jpg"
    generate_waterfall().save(wf, "JPEG", quality=94, optimize=True)
    generate_cliff().save(cliff, "JPEG", quality=94, optimize=True)
